...
def ingest_orders(run_id: str | None = None) -> str:
    ...
    raw = read_csv_landing("orders_raw.csv")
    raw = stamp_ingest_metadata(raw, source_file="orders_raw.csv")
    write_staging("bronze", "orders", raw, run_id)
    return promote("bronze", "orders", run_id)
